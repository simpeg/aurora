...
def fast_arange(t0, n_samples, sampling_rate):
    t0 = np.datetime64(t0)
    dt = 1. / sampling_rate
...
def slow_comprehension(t0, n_samples, sampling_rate):
    t0 = np.datetime64(t0)
    dt = 1. / sampling_rate
    time_vector_seconds = dt * np.arange(n_samples)
    time_vector_nanoseconds = (np.round(1e9 * time_vector_seconds)).astype(int)
    time_index = np.array([t0 + np.timedelta64(x, 'ns') for x in time_vector_nanoseconds])
    return time_index
...
def decide_time_axis_method(sampling_rate):
    dt = 1. / sampling_rate
...
def make_time_axis(t0, n_samples, sampling_rate):
    method = decide_time_axis_method(sampling_rate)
    time_axis = TIME_AXIS_GENERATOR_FUNCTIONS[method](t0, n_samples, sampling_rate)
    return time_axis
...
def test_generate_time_axis(t0, n_samples, sampling_rate):
    ...
    # <SLOW>
    tt = time.time()
    time_index_1 = slow_comprehension(t0, n_samples, sampling_rate)
    ...
    # <FAST>
    tt = time.time()
    time_index_2 = fast_arange(t0, n_samples, sampling_rate)
...
#def make_time_axis_method1(t0, n_samples, sampling_rate):
    #pass
