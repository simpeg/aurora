...
def tfs_nearly_equal(tf1:TF, tf2:TF) -> bool:
    ...
    """
    cond1 = np.isclose(tf2.residual_covariance, tf1.residual_covariance, atol=1e-26, rtol=1e-30).all()
    cond2 = np.isclose(tf2.transfer_function_error, tf1.transfer_function_error, atol=1e-14, rtol=1e-15).all()
    cond3 = np.isclose(tf2.transfer_function, tf1.transfer_function, atol=1e-14, rtol=1e-15).all()
