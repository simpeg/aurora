...
def simple_coherence_weights(
    ...
    """
    Note 1: Extremely high coherence can be due to noise, however, that is a different "flavor" of
    noise than that which causes low coherence, so for now, will avoid mixing lower bounds and upper bounds in
    the same method.
    ...
    Thus we can use any of the following pairs:
    (ex, hy), (ey, hx), (ex, ry), (ey, rx), (hx, rx), (hy, ry)


    Note that when solving the "ex" equation, we could apply the weights for:
    [ex, hy], [hx, rx], [hy, ry],
    ...
    Strategy:
        Start by computing all lower cutoff booleans

        If too many data points are cut out, we can:
            1. Boost the thresholds (iteratively) until we are cutting less than max_reject_fraction
            or
            2. Apply a quantile or rank to the data, and reject from worst to best up to the threshold
            On lower only cuts this is not too bad, on mixed lower and upper cutoffs this might be complicated
    ...
    key = f"{ch1}_{ch2}"
    n_obs = simple_coherences.time.shape[0]
    weights = np.ones(n_obs)
    simple_coherence = simple_coherences[key]

    if reject == "below":
        zero_weights = simple_coherence < threshold
        threshold_str = f"Applying {ch1}_{ch2} < threshold={threshold}"
    elif reject == "above":
        zero_weights = simple_coherence > threshold
        threshold_str = f"Applying {ch1}_{ch2} > threshold={threshold}"
    ...
    # Note this must eventually work with cumulative weights, not individual!
    if reject_fraction > max_reject_fraction:
        ...
        logger.warning(msg)
        zero_weights = simple_coherence < np.quantile(
            simple_coherence, max_reject_fraction
        )
        n_reject = zero_weights.sum()
    # keep track of lowest and highest values per time window
    # rankings += np.argsort(simple_coherence)

    msg = (
        f"Rejecting {n_reject} of {n_obs} spectrogram samples based on simple coherence"
    )
    msg = f"{msg} \n {threshold_str}"
    logger.info(msg)
    weights[zero_weights] = 0.0
    ...
    #         cutoffs["hx_rx"] = {}
    #         cutoffs["hx_rx"]["lower"] = 0.3
    # #        cutoffs["hx_rx"]["upper"] = 0.999
    #         cutoffs["hy_ry"] = {}
    #         cutoffs["hy_ry"]["lower"] = 0.3
    # #        cutoffs["hy_ry"]["upper"] = 0.999
    #
    #
    #     # # TODO: define conjugate channel pairs via keys to cutoffs if not provided
    ...
    #         weights[key] = np.ones(n_obs)
    #         # weights[key] = {}
    #         # weights[key]["lower"] = np.ones(n_obs)
    #         # weights[key]["upper"] = np.ones(n_obs)
    #
    #     # cumulative_weights = np.ones(n_obs)
    #     cumulative_rankings = np.zeros(n_obs)
    #
    #
    #     # Define the channel pair
    #     for key in cutoffs.keys():
    #         simple_coherence = simple_coherences[key]
    #         cumulative_rankings += np.argsort(simple_coherence) # keep track of lowest and highest values per time window
    #         # Apply lower weights
    #         zero_weights = simple_coherence < cutoffs[key]["lower"]
    #         n_reject = zero_weights.sum()
    #         reject_fraction = 1.0 * n_reject / n_obs
    ...
    #     msg += f" -- using {max_reject_fraction} quantile instead\n"
    #     logger.warning(msg)
    #     zero_weights = simple_coherence < np.quantile(simple_coherence, max_reject_fraction)
    #     n_reject = zero_weights.sum()
    ...
    # logger.info(msg)
    # weights[key][zero_weights] = 0.0

    # # Apply upper weights
    # threshold_upper = cutoffs[key]["upper"]
    ...
    # TODO: Add rejection fraction logic here
    # total_n_rejected = cumulative_weights.sum()
    # if reject_fraction > max_reject_fraction:
    #     msg = f"\nRejection fraction {reject_fraction} greater than max allowed {max_reject_fraction}\n"
    #     msg += f" -- corresponding to {n_reject}/{n_obs} observations\n"
    #     msg += f" -- using {max_reject_fraction} quantile instead\n"
...
def coherence_weights_jj84(
    ...
    Main idea: Iteratively remove each observation in turn and check whether that partial
      ...
      This leads to a score for each observation.  We can remove data that say move the estimate down.
